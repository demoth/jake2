...
// $Id: SV_WORLD.java,v 1.2 2004-01-20 22:25:06 rst Exp $
...
public class SV_WORLD extends SV_CCMDS {
	...
	*/
	...
	public static void SV_LinkEdict(edict_t ent) {
		...
		num_leafs = CM.CM_BoxLeafnums(ent.absmin, ent.absmax, 
		leafs, MAX_TOTAL_ENT_LEAFS, iw);
		topnode = iw.i;
			...
			if (area!=0) { // doors may legally straggle two areas,
				...
				if (ent.areanum!=0 && ent.areanum != area) {
					if (ent.areanum2!=0 && ent.areanum2 != area && sv.state == ss_loading)
						Com.DPrintf("Object touching 3 areas at " + ent.absmin[0] 
						+ " "+ ent.absmin[1]+ " "+ ent.absmin[2] + "\n");
					ent.areanum2 = area;
				}
				else
					ent.areanum = area;
