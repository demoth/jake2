...
public class CM extends Game
{
	...
	public static mapsurface_t map_surfaces[]=
		new mapsurface_t[MAX_MAP_TEXINFO];
	static {
	...
	public static qfiles.dvis_t map_vis=
		new qfiles.dvis_t(ByteBuffer.wrap(map_visibility));
	...
	public static qfiles.dareaportal_t map_areaportals[]=
		new qfiles.dareaportal_t[MAX_MAP_AREAPORTALS];
	...
	*/
	...
	==================
	*/
	public static cmodel_t CM_LoadMap(
		String name,
		boolean clientload,
		intwrap checksum)
	{
		...
		for (i= 0; i < count; i++)
		{
			in=
				new qfiles.dmodel_t(
					ByteBuffer.wrap(
						cmod_base,
						i * qfiles.dmodel_t.SIZE + l.fileofs,
						qfiles.dmodel_t.SIZE));
			out= map_cmodels[i];
		...
		for (i= 0; i < count; i++)
		{
			...
			*/
			...
			if (debugloadmap)
			{
				Com.DPrintf(
					"|%20s|%20s|%6i|%6i|\n",
					new Vargs().add(out.c.name).add(out.rname).add(
						out.c.value).add(
						out.c.flags));
			}
		...
		for (i= 0; i < count; i++)
		{
			in=
				new qfiles.dnode_t(
					ByteBuffer.wrap(
						cmod_base,
						qfiles.dnode_t.SIZE * i + l.fileofs,
						qfiles.dnode_t.SIZE));
			out= map_nodes[i];
		...
		for (i= 0; i < count; i++)
		{
			in=
				new qfiles.dbrush_t(
					ByteBuffer.wrap(
						cmod_base,
						i * qfiles.dbrush_t.SIZE + l.fileofs,
						qfiles.dbrush_t.SIZE));
			out= map_brushes[i];
	...
	public static void CMod_LoadLeafs(lump_t l)
	{
		...
		for (i= 0; i < count; i++)
		{
			in=
				new qfiles.dleaf_t(
					cmod_base,
					i * qfiles.dleaf_t.SIZE + l.fileofs,
					qfiles.dleaf_t.SIZE);
		...
		for (i= 0; i < count; i++)
		{
			in=
				new qfiles.dplane_t(
					ByteBuffer.wrap(
						cmod_base,
						i * qfiles.dplane_t.SIZE + l.fileofs,
						qfiles.dplane_t.SIZE));
	...
	*/
	public static void CMod_LoadLeafBrushes(lump_t l)
	{
		...
		ByteBuffer bb=
			ByteBuffer.wrap(cmod_base, l.fileofs, count * 2).order(
		...
		for (i= 0; i < count; i++)
		{
			...
			in=
				new qfiles.dbrushside_t(
					ByteBuffer.wrap(
						cmod_base,
						i * qfiles.dbrushside_t.SIZE + l.fileofs,
		...
		for (i= 0; i < count; i++)
		{
			...
			in=
				new qfiles.darea_t(
					ByteBuffer.wrap(
						cmod_base,
						i * qfiles.darea_t.SIZE + l.fileofs,
						qfiles.darea_t.SIZE));
			out= map_areas[i];
		...
		for (i= 0; i < count; i++)
		{
			in=
				new qfiles.dareaportal_t(
					ByteBuffer.wrap(
						cmod_base,
						i * qfiles.dareaportal_t.SIZE + l.fileofs,
	...
	*/
	...
	public static int CM_BoxLeafnums_headnode(
		float[] mins,
		float[] maxs,
		int list[],
		int listsize,
	...
	public static int CM_BoxLeafnums(
		float[] mins,
		float[] maxs,
		int list[],
		int listsize,
	...
	*/
	...
	==================
	*/
	public static int TransformedPointContents(
		float[] p,
		int headnode,
		float[] origin,
		float[] angles)
	...
	*/
	...
	================
	*/
	public static void CM_ClipBoxToBrush(
		float[] mins,
		float[] maxs,
		float[] p1,
		float[] p2,
	...
	================
	*/
	public static void CM_TestBoxInBrush(
		float[] mins,
		float[] maxs,
		float[] p1,
		trace_t trace,
	...
	*/
	...
	==================
	*/
	public static void CM_RecursiveHullCheck(
		int num,
		float p1f,
		float p2f,
		float[] p1,
	...
	==================
	*/
	public static trace_t BoxTrace(
		float[] start,
		float[] end,
		float[] mins,
		float[] maxs,
		...
		int brushmask)
	{
		...
		if (start[0] == end[0] && start[1] == end[1] && start[2] == end[2])
		{
			...
			numleafs=
				CM_BoxLeafnums_headnode(c1, c2, leafs, 1024, headnode, tn);
			topnode= tn.i;
	...
	*/
	...
	public static byte[] CM_ClusterPVS(int cluster)
	{
		if (cluster == -1)
			...
			CM_DecompressVis(
				map_visibility,
				map_vis.bitofs[cluster][DVIS_PVS],
				pvsrow);
		return pvsrow;
	}
	...
	public static byte[] CM_ClusterPHS(int cluster)
	{
		if (cluster == -1)
			...
			CM_DecompressVis(
				map_visibility,
				map_vis.bitofs[cluster][Defines.DVIS_PHS],
				phsrow);
		return phsrow;
	}
	...
	*/
	public static int CM_WriteAreaBits(byte buffer[], int area)
	{
		...
		{ // for debugging, send everything
			...
			//Arrays.fill(buffer, 0, bytes, (byte) 0xff);
	...
	*/
	public static void CM_ReadPortalState(RandomAccessFile f)
	{
		...
		//todo: check order (up or downcounting)
		for (int n= 0; n < portalopen.length; n++)
