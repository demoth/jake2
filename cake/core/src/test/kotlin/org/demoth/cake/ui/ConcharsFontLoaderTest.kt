package org.demoth.cake.ui

import org.junit.Assert.assertEquals
import org.junit.Test

class ConcharsFontLoaderTest {
    @Test
    fun mapsAllGlyphCellsInRowMajorOrder() {
        val cells = ConcharsFontLoader.mapCells(256, 256)

        assertEquals(256, cells.size)
        assertEquals(0, cells.first().x)
        assertEquals(0, cells.first().y)

        assertEquals(240, cells[15].x)
        assertEquals(0, cells[15].y)

        assertEquals(0, cells[16].x)
        assertEquals(16, cells[16].y)

        assertEquals(240, cells.last().x)
        assertEquals(240, cells.last().y)
    }

    @Test(expected = IllegalArgumentException::class)
    fun rejectsUnexpectedAtlasSize() {
        ConcharsFontLoader.mapCells(128, 128)
    }
}
